...
import { BookOpen, Video, FileText, CheckCircle2, PlayCircle, Code2 } from 'lucide-react';
import Link from 'next/link';

export default async function LearningDashboard() {
  // Fetch the logged-in user from Clerk
  const user = await currentUser();

  // If they somehow bypass middleware but aren't logged in, force them out
  if (!user) {
    redirect('/');
  }

  // Get their first name (or fallback to "Student")
      ...
      <main className="flex-grow max-w-7xl mx-auto px-6 py-20 w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {/* Example Course Card (In the future, this will map over their purchased courses) */}
            <div className="bg-white rounded-3xl p-6 border border-stone-200 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden">
              ...
              <div className="flex gap-4">
                <button className="flex-1 py-3 rounded-xl bg-stone-900 text-white font-bold flex items-center justify-center gap-2 hover:bg-stone-800 transition-colors">
                  <PlayCircle size={18} /> Join Live Class
                </button>
                <button className="flex-1 py-3 rounded-xl border border-stone-200 text-stone-700 font-bold flex items-center justify-center gap-2 hover:bg-stone-50 transition-colors">
          ...
          {/* Right Column: Resources & Community */}
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl p-6 border border-stone-200 shadow-sm">
              ...
              <div className="space-y-3">
                ...
                <a href="#" className="flex items-center gap-3 p-3 rounded-xl hover:bg-stone-50 transition-colors border border-transparent hover:border-stone-200">
                  <div className="p-2 bg-blue-100 rounded-lg text-blue-700"><Code2 size={16} /></div>
                  <div>
                    <p className="text-sm font-bold text-stone-800">Python Setup Guide</p>
                    <p className="text-xs text-stone-500">Installation Instructions</p>
                  </div>
                </a>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-amber-500 to-amber-600 rounded-2xl p-6 text-white shadow-lg">
              <h4 className="font-bold text-xl mb-2">Private Community</h4>
              <p className="text-amber-100 text-sm mb-4">Ask questions, share code, and network with other students.</p>
              <button className="w-full py-3 rounded-xl bg-white text-stone-900 font-bold hover:bg-amber-50 transition-colors">
                Join Discord Server
              </button>
            </div>
          </div>
