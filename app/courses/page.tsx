          ...
          {activeCourses.map((course) => (
            <div key={course.id} className="glass-panel p-8 rounded-3xl border border-stone-200 bg-white shadow-sm hover:shadow-xl transition-shadow flex flex-col h-full group">
              <div className="flex justify-between items-start mb-6">
                <span className="px-3 py-1 bg-red-100 text-red-600 font-bold rounded-full text-xs">{course.statusText}</span>
                <span className="text-2xl font-black text-stone-900">{course.pricing?.currentPrice}</span>
              </div>
              ...
              {/* This link magically takes them to /courses/python-beginners */}
