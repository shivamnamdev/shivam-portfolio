  ...
  return (
    <section className="w-full relative scroll-mt-32 z-10" id="curriculum">
      <div className="text-center mb-12">
        <h2 className="text-4xl font-display font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400 mb-4">Course Curriculum</h2>
        <p className="text-slate-400">Everything you need to go from Manual QA to AI Automation Lead.</p>
      </div>
      ...
      <div className="flex flex-col gap-4 max-w-4xl mx-auto">
        {modules.map((mod, i) => {
          ...
          return (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className={`glass-panel rounded-2xl overflow-hidden transition-all ${isActive ? 'border-amber-500/50 bg-amber-500/5' : ''}`}>
              <button onClick={() => setOpenIndex(isActive ? null : i)} className="w-full p-6 flex items-center justify-between text-left">
                <h4 className={`text-xl font-bold ${isActive ? 'text-amber-400' : 'text-slate-200'}`}>{mod.title}</h4>
                <motion.div animate={{ rotate: isActive ? 180 : 0 }}><ChevronDown className="text-slate-400"/></motion.div>
              </button>
              <AnimatePresence>
                {isActive && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                    <ul className="p-6 pt-0 space-y-4 border-t border-white/5 mt-2">
                      {mod.lessons.map((lesson, idx) => (
                        <li key={idx} className="flex items-center gap-3 text-slate-300">
                          <Code2 size={16} className="text-amber-500 opacity-70" /> {lesson}
                        </li>
                      ))}
                    </ul>
                  </motion.div>
