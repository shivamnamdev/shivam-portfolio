  ...
  return (
    <section className="w-full pt-16 md:pt-28 flex flex-col lg:flex-row items-center gap-12 relative z-10" id="hero">
      ...
      <div className="w-full lg:w-1/2 flex flex-col items-center lg:items-start text-center lg:text-left">
        ...
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-5xl md:text-6xl lg:text-7xl font-display font-black tracking-tighter text-white leading-tight mb-6 drop-shadow-md">
        ...
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-lg md:text-xl text-stone-400 font-medium max-w-xl leading-relaxed mb-10">
          {course.vsl.subheadline}
        </motion.p>

        <motion.button onClick={scrollToCohorts} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="px-8 py-4 rounded-full bg-gradient-to-r from-amber-500 to-amber-600 text-black font-black text-lg flex items-center justify-center gap-3 transition-all hover:scale-105 hover:shadow-[0_0_30px_rgba(245,158,11,0.3)] w-full sm:w-auto group">
      ...
      <div className="w-full lg:w-1/2 flex justify-center lg:justify-end relative">
        ...
        <motion.div 
          ...
          className="relative w-full max-w-md rounded-3xl overflow-hidden border border-white/10 shadow-[0_0_50px_rgba(245,158,11,0.2)] transform hover:scale-[1.02] hover:-rotate-1 transition-all duration-300 bg-[#0a0a0a]"
        >
          <img src={course.vsl.flyerUrl} alt="Course Overview" className="w-full h-auto object-cover opacity-90 hover:opacity-100 transition-opacity" />
        </motion.div>
      </div>
    </section>
  );
